...
const validateHashtag = (value) => {
  ...
  const duplicatedHashtags = hashtags.filter((hashtag, index, hashtagsArray) => hashtagsArray.indexOf(hashtag) !== index);
  return (hashtagIsValid && hashtags.length <= MAX_HASHTAG_COUNT && duplicatedHashtags.length === 0);
};

const validateDescription = (value) => checkStringLength(value, MAX_COMMENT_LENGTH);
